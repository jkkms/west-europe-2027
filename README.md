...
> 글이나 일정만 고쳤다면(= `index.html`만 바꿨다면) 그냥 push하면 됩니다. 새로고침하면
> 최신 화면이 나옵니다.
>
> 다만 **좌석 배치도나 아이콘 파일을 바꿔치기했다면** `sw.js` 맨 위의 `VERSION`을
> `'v1'` → `'v2'`처럼 올려주세요. 이 파일들은 한 번 받아두면 계속 재사용해서,
> 안 올리면 이미 앱을 열어본 기기에 옛 그림이 계속 나옵니다.
...
1. **바르셀로나 → 파리 직행 TGV 미예약** — 2026-09-30 판매 개시. 편수가 적어 최우선.
2. 2/7은 2027년 2월 첫째 일요일이라 오르세가 무료 개방일이며, 이날은 시간 예약이 의무입니다.
3. 피렌체 중앙시장의 일요일(2/14) 영업 여부 미확인.
